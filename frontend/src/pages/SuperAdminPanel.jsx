...
import { Building2, Users, Activity, CheckCircle, XCircle, AlertCircle, TrendingUp, Eye, Edit, Trash2, UserCheck, Ban, Search, Plus } from 'lucide-react';

const SuperAdminPanel = () => {
  const [activeTab, setActiveTab] = useState('overview');
  const [hospitals, setHospitals] = useState([
    { id: 1, name: 'City General Hospital', admin: 'Dr. Sarah Johnson', status: 'active', users: 145, created: '2024-01-15', revenue: 45000 },
    { id: 2, name: 'Metro Medical Center', admin: 'Dr. Michael Chen', status: 'active', users: 89, created: '2024-02-20', revenue: 32000 },
    { id: 3, name: 'Riverside Clinic', admin: 'Dr. Emily Davis', status: 'pending', users: 0, created: '2024-11-25', revenue: 0 },
    { id: 4, name: 'Northside Health', admin: 'Dr. James Wilson', status: 'suspended', users: 67, created: '2023-12-10', revenue: 21000 },
  ]);
  
  const [admins, setAdmins] = useState([
    { id: 1, name: 'Dr. Sarah Johnson', email: 'sarah.j@citygeneral.com', hospital: 'City General Hospital', status: 'active', lastLogin: '2024-11-28' },
    { id: 2, name: 'Dr. Michael Chen', email: 'michael.c@metromed.com', hospital: 'Metro Medical Center', status: 'active', lastLogin: '2024-11-27' },
    { id: 3, name: 'Dr. Emily Davis', email: 'emily.d@riverside.com', hospital: 'Riverside Clinic', status: 'pending', lastLogin: 'Never' },
    { id: 4, name: 'Dr. James Wilson', email: 'james.w@northside.com', hospital: 'Northside Health', status: 'suspended', lastLogin: '2024-10-15' },
  ]);
  ...
  const handleApproveHospital = (id) => {
    ...
    setAdmins(admins.map(a => 
      a.hospital === hospitals.find(h => h.id === id)?.name ? { ...a, status: 'active' } : a
    ));
  ...
  const handleSuspendHospital = (id) => {
    ...
    setAdmins(admins.map(a => 
      a.hospital === hospitals.find(h => h.id === id)?.name ? { ...a, status: 'suspended' } : a
    ));
  ...
  const handleAddHospital = () => {
    if (!newHospital.name || !newHospital.adminName || !newHospital.adminEmail) {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-blue-50 via-white to-blue-50">
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center space-x-3">
              ...
              <div>
                ...
                <p className="text-sm text-blue-600">Platform-wide management & oversight</p>
              </div>
            </div>
            <div className="flex items-center space-x-2 bg-blue-50 px-4 py-2 rounded-lg border border-blue-300">
              <div className="w-2 h-2 bg-green-500 rounded-full animate-pulse"></div>
              <span className="text-sm text-blue-700 font-medium">SUPER_ADMIN</span>
            </div>
          </div>
        </div>
      </div>
        ...
        <div className="max-w-7xl mx-auto px-6 py-8">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
            <div className="bg-gradient-to-br from-blue-500 to-blue-600 rounded-xl p-6 shadow-lg">
              ...
              <div className="text-3xl font-bold text-white mb-1">{metrics.totalHospitals}</div>
            ...
            <div className="bg-gradient-to-br from-green-500 to-green-600 rounded-xl p-6 shadow-lg">
              ...
              <div className="text-3xl font-bold text-white mb-1">{metrics.activeHospitals}</div>
            ...
            <div className="bg-gradient-to-br from-yellow-500 to-yellow-600 rounded-xl p-6 shadow-lg">
              ...
              <div className="text-3xl font-bold text-white mb-1">{metrics.pendingApprovals}</div>
            ...
            <div className="bg-white rounded-xl p-6 border border-blue-200 shadow-sm">
              ...
              <div className="space-y-3">
                ...
                <div className="flex justify-between items-center">
                  ...
                  <span className="text-red-600 font-semibold">{metrics.suspendedHospitals}</span>
                </div>
              </div>
            </div>
          </div>
            ...
            <div className="overflow-x-auto">
              <table className="w-full">
                <thead className="bg-blue-50">
                  <tr>
                    <th className="px-6 py-4 text-left text-xs font-semibold text-blue-700 uppercase tracking-wider">Hospital</th>
                    <th className="px-6 py-4 text-left text-xs font-semibold text-blue-700 uppercase tracking-wider">Admin</th>
                    <th className="px-6 py-4 text-left text-xs font-semibold text-blue-700 uppercase tracking-wider">Users</th>
                    <th className="px-6 py-4 text-left text-xs font-semibold text-blue-700 uppercase tracking-wider">Status</th>
                    <th className="px-6 py-4 text-left text-xs font-semibold text-blue-700 uppercase tracking-wider">Created</th>
                    <th className="px-6 py-4 text-left text-xs font-semibold text-blue-700 uppercase tracking-wider">Revenue</th>
                    <th className="px-6 py-4 text-left text-xs font-semibold text-blue-700 uppercase tracking-wider">Actions</th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-blue-100">
                  {filteredHospitals.map((hospital) => (
                    <tr key={hospital.id} className="hover:bg-blue-50 transition-colors">
                      <td className="px-6 py-4">
                        <div className="font-medium text-gray-800">{hospital.name}</div>
                      </td>
                      <td className="px-6 py-4 text-gray-600">{hospital.admin}</td>
                      <td className="px-6 py-4 text-gray-600">{hospital.users}</td>
                      <td className="px-6 py-4">
                        <StatusBadge status={hospital.status} />
                      </td>
                      <td className="px-6 py-4 text-gray-600">{hospital.created}</td>
                      <td className="px-6 py-4 text-gray-600">${hospital.revenue.toLocaleString()}</td>
                      <td className="px-6 py-4">
      ...
      {activeTab === 'admins' && (
        <div className="max-w-7xl mx-auto px-6 py-8">
          <div className="flex justify-between items-center mb-6">
            <h2 className="text-2xl font-bold text-gray-800">Hospital Admins</h2>
            <div className="relative">
              <Search className="absolute left-3 top-1/2 transform -translate-y-1/2 w-5 h-5 text-gray-400" />
              <input
                type="text"
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            {filteredAdmins.map((admin) => (
              <div key={admin.id} className="bg-white rounded-xl p-6 border border-blue-200 shadow-sm hover:border-blue-400 hover:shadow-md transition-all">
                ...
                <div className="space-y-2 text-sm">
                  <div className="flex justify-between">
                    ...
                    <span className="text-gray-800 font-medium">{admin.hospital}</span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-gray-500">Last Login:</span>
                    <span className="text-gray-800 font-medium">{admin.lastLogin}</span>
                  </div>
                </div>
              </div>
            ))}
            ...
            <div className="space-y-4">
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <p className="text-gray-800 font-medium">{selectedHospital.admin}</p>
                </div>
                <div>
                ...
                <div>
                  ...
                  <p className="text-gray-800 font-medium">{selectedHospital.users}</p>
                </div>
                <div>
                  <label className="text-sm text-gray-500">Created Date</label>
                  <p className="text-gray-800 font-medium">{selectedHospital.created}</p>
                </div>
                <div>
                  <label className="text-sm text-gray-500">Monthly Revenue</label>
                  <p className="text-gray-800 font-medium">${selectedHospital.revenue.toLocaleString()}</p>
                </div>
              </div>
            </div>
          </div>
      ...
      {showAddModal && (
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-xl max-w-2xl w-full p-6 border border-blue-300 shadow-xl max-h-[90vh] overflow-y-auto">
            <div className="flex justify-between items-start mb-6">
              <div>
                <h3 className="text-2xl font-bold text-gray-800">Add New Hospital</h3>
                <p className="text-sm text-gray-500 mt-1">Fill in the details to register a new hospital</p>
              </div>
              <button
                ...
                <div className="flex items-start space-x-2">
                  ...
                  <div className="text-sm text-blue-800">
                    ...
                    <ul className="list-disc list-inside space-y-1 text-blue-700">
                      <li>The hospital will be created with "Pending" status</li>
                      <li>Admin credentials will be sent via email</li>
                      <li>You'll need to approve the hospital before it becomes active</li>
                    </ul>
                  </div>
                </div>
              </div>
